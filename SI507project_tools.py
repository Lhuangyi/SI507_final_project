...
from runserver import db, University, Course , Instructor
...
class Spider:
    ...
    for url in new_urls:
        ...
        if url == 'https://www.coursera.org/learnquest':
            ...
            # university_description = soup.find('p')
            # if university_description:
            #     university_description = soup.find('p').get_text()
            #     data.append(university_description)
            # else:
            #     data.append(None)
            ...
            #instructor_node = soup.find('div', {'class':'bt3-row'})
            all_instructors = soup.find_all('h4', {'class': 'instructorName headline-1-text'})
            for i in all_instructors:
                name = i.get_text()
                #position = node.find('div', {'class':'body-1-text'}).get_text()
                instructors.append(name)
                #instructor_position.append(position)
